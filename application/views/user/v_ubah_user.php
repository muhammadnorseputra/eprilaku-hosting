<section class="content" style="padding:0px;">
    <div class="panel panel-primary">
        <div class="panel-heading">Ubah <?= $par == 0 ? 'User' : 'Admin' ?> <a href="javascript:void(0)" class="pull-right" style="color:white;font-weight: bold;" onclick="$('.close').click()">[ x ]</a></div>
        <div class="panel-body">
            <form method="post" id="frmUbahUser">
                <div class="form-horizontal">
                    <input type="hidden" name="id_dd_user" value="<?= $detail['id_dd_user'] ?>">
                    <?php 
                        //if ($par === 0) {
                        if ($par == 0) { 
                    ?>
                            ...
                            <div class="col-sm-6">
                                <select name="jabatan" class="form-control" width="200">
                                    ...
                                    foreach ($jabatan as $dtJab) {
                                        ?>
                                        <option value="<?= $dtJab['kodejab'] ?>" <?= $dtJab['kodejab'] === $detail['jabatan'] ? 'selected' : ''; ?>><?= $dtJab['jabatan'] ?></option>
                                        <?php
                                    }
                                    ?>
                                </select>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-6">
                            <input type="text" class="form-control" name="password" id="password" value="<?= base64_decode($detail['password']) ?>">
                        </div>
                    </div>
                    <div class="form-group">
...
<script>
    $("#frmUbahUser").on('submit', (function (e) {
        e.preventDefault();
